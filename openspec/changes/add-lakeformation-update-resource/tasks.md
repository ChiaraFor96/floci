...
- [ ] 1.1 Verify `UpdateResource` request fields, optional-field semantics, response shape, and error codes against the AWS SDK model
- [ ] 1.2 Add failing service tests for role updates, omitted optional values, missing resources, and Region isolation
...
- [ ] 2.4 Add the thin `POST /UpdateResource` controller route using existing parsing and AWS exception mapping

## 3. Verify Compatibility

- [ ] 3.1 Run `./mvnw test -Dtest=LakeFormationServiceTest`
- [ ] 3.2 Add and run an AWS SDK integration lifecycle test in `LakeFormationIntegrationTest`
